...
diff.data = diff(time.data)

plot(diff.data)
acf(diff.data)

diff.pts = c(1:length(diff.data))
diff.pts = c(ma.pts - min(diff.pts))/max(diff.pts)

# moving average
diff.fit = ksmooth(diff.pts, diff.data, kernel = "box")
...
diff.x1 = diff.pts
diff.x2 = diff.pts^2
diff.lm.fit = lm(data$Close~diff.x1+diff.x2)
summary(diff.lm.fit)
#diff.fit.lm = ts(fitted(diff.lm.fit),start=2010,frequency=12)
#ts.plot(diff.data,ylab="Close")
