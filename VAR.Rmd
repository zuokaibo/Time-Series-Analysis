...
##### code below is for statistical test on models with aic and with bic.
...
ts.merge = merge(xts(total.sales, datte), xts(new.orders, datte), join = "inner")

colnames(ts.merge) = c("total.sales", "new.orders")
n = dim(ts.merge)[1]
...
train_data_varx = head(diff_data, -9)
test_data_varx = tail(diff_data, 9)
...
```{r}
varx.model = VAR(train_data_varx, type = "both", p = 7, exogen = train_data_varx[, 1] )
#summary(varx.model)
varx.model
```
...
```{r}
varx.model = VAR(train_data_varx, type = "both", p = 2, exogen = train_data_varx[, 2] )
#summary(varx.model)
varx.model
```
...
```{r}
#final model for S2
order.spec = ugarchspec(variance.model=list(model = "eGARCH", submodel = "EGARCH",garchOrder=c(4,3)), mean.model=list(armaOrder=c(0,0), include.mean=T), distribution.model="std")
...
pred.test_var <- as.vector(predict(var.aic.model,n.ahead=9))
s2.pred_var=pred.test_var$fcst$total.sales[,1]+total.sales.test[1:nfore]
```
