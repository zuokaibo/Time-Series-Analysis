...
```{r}
allorders.sales = function(ts_data, max_p, max_d, max_q){
  orders = data.frame()
  for (p in 0:max_p){
    for (d in 0:max_d){
      for (q in 0:max_q) {
...
head(allorders.sales(sales.train,8,1,8), 5)
```
...
plot(residuals, main="Residuals", type="l")
```

```{r}
plot(squared_residuals, main="Squared Residuals", type="l")
```
...
```{r}
acf(squared_residuals, main="ACF of Residuals")
```
...
```{r}
pacf(squared_residuals, main="PACF of Residuals")
```
...
## The code above gave the error when i gave arima order (2,2), but if i changed order to (2,1), then error messege disappeared. the code above is apply to 1st differenced order of new orders. 
